                    ...
                    <div class="input-group">
                        ...
                        <select class="form-control" name="position_id">
                            <?php if(is_array($positions)): foreach($positions as $key=>$position): ?><option value="<?php echo ($position["id"]); ?>"
                                <?php if($position['id'] == $position_id): ?>selected = "selected"<?php endif; ?>
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-hover singcms-table">
                        ...
                        <?php if(is_array($contents)): $i = 0; $__LIST__ = $contents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$content): $mod = ($i % 2 );++$i;?><tr>
                                <td><input size=4 type='text' name="listorder[<?php echo ($content["id"]); ?>]"
                                           value="<?php echo ($content["listorder"]); ?>"/></td>
                                ...
                                <td>
                                    <!--将status转化成文字-->
                                    <span attr-status="<?php if( $content['status'] == 1): ?>0
                                    <?php else: ?>
                                    1<?php endif; ?>" attr-id="<?php echo ($content["id"]); ?>" class="sing_cursor singcms-on-off"
                                    id="singcms-on-off" ><?php echo (getMenuStatus($content["status"])); ?></span>
                                </td>
                                <td>
                                    <span class="sing_cursor glyphicon glyphicon-edit" aria-hidden="true"
                                          id="singcms-edit" attr-id="<?php echo ($content["id"]); ?>"></span>
                                    <a href="javascript:void(0)" id="singcms-delete" attr-id="<?php echo ($content["id"]); ?>"
                                       attr-message="删除">
